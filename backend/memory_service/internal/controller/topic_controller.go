...
func NewTopicController(db *sql.DB, topicRepo *repository.TopicRepository, memberAccessRepo *repository.MemberAccessRepository) *TopicController {
	return &TopicController{db: db, topicRepo: topicRepo, memberAccessRepo: memberAccessRepo}
}

// CreateTopic handles the business logic for creating a new topic
func (c *TopicController) CreateTopic(ctx context.Context, req *mpb.CreateTopicRequest) (*mpb.CreateTopicResponse, error) {
	// TODO: continue here, lets put the member access in the cache so we can check if the user has access to the subject
	// we need to check access to this subject
	// for other api, we need to check that topic is_master (editing documents, memory and question)

	// access:{user_id}:{subject_id} -> {role} for access
	// topic_map:{topic_id} -> {subject_id} -> {is_master} for topic

}

// GetTopicById handles the business logic for retrieving a topic by ID
func (c *TopicController) GetTopicById(ctx context.Context, req *mpb.GetTopicByIdRequest) (*mpb.GetTopicByIdResponse, error) {
